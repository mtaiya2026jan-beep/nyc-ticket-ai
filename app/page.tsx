        ...
        <div style={{height:56, minHeight:56, background:'var(--bg2)', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', padding:'0 24px', gap:16}}>
          ...
          <div style={{marginLeft:'auto', display:'flex', gap:12}}>
            <div style={{display:'flex', alignItems:'center', gap:6, background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:6, padding:'5px 10px', fontSize:11, color:'var(--text2)'}}>
              <i className="ti ti-database" style={{fontSize:13}} aria-hidden /> <span style={{color:'var(--text)', fontWeight:500}}>数据库 8.2M+</span>
            </div>
          </div>
        </div>
                ...
                {appeals.map((a:any)=>(
                  <div key={a.id} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:20}}>
                    <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
                      ...
                      <div style={{display:'flex',gap:6,flexWrap:'wrap'}}><button onClick={()=>{const blob=new Blob([a.appeal_text],{type:'text/plain'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download='申诉书_'+(a.summons_number||a.id)+'.txt';link.click();URL.revokeObjectURL(url)}} style={{background:'var(--surface2)',color:'var(--text)',border:'1px solid var(--border)',borderRadius:8,padding:'6px 12px',cursor:'pointer',fontSize:11,fontWeight:600,whiteSpace:'nowrap'}}>TXT</button><button onClick={()=>{import('@/lib/exportAppeal').then(m=>m.exportAppealAsDocx(a.appeal_text,a.summons_number||a.id))}} style={{background:'var(--surface2)',color:'var(--text)',border:'1px solid var(--border)',borderRadius:8,padding:'6px 12px',cursor:'pointer',fontSize:11,fontWeight:600,whiteSpace:'nowrap'}}>Word</button><button onClick={()=>{import('@/lib/exportAppeal').then(m=>m.exportAppealAsPdf(a.appeal_text,a.summons_number||a.id))}} style={{background:'var(--accent)',color:'#000',border:'none',borderRadius:8,padding:'6px 12px',cursor:'pointer',fontSize:11,fontWeight:600,whiteSpace:'nowrap'}}>PDF</button></div>
                    </div>
